...
$conn = mysqli_connect("localhost", "root", "", "ojt_portal");
...
if (isset($_POST['upload_resume'])) {
    ...
    if ($_FILES["resume_file"]["size"] > 5000000) {
        ...
        if (move_uploaded_file($_FILES["resume_file"]["tmp_name"], $target_file)) {
            $check_query = "SELECT * FROM student_resumes WHERE student_id = '$student_id'";
            $check_result = mysqli_query($conn, $check_query);
            
            if (!$check_result) {
                $upload_error = "Database error: " . mysqli_error($conn);
            } else if (mysqli_num_rows($check_result) > 0) {
                $update_query = "UPDATE student_resumes SET resume_path = '$target_file', upload_date = NOW() WHERE student_id = '$student_id'";
                ...
                $insert_query = "INSERT INTO student_resumes (student_id, resume_path, upload_date) VALUES ('$student_id', '$target_file', NOW())";
...
$resume_query = "SELECT * FROM student_resumes WHERE student_id = '$student_id'";
                ...
                <div class="resume-card">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Uploaded on: <?php echo date('F j, Y', strtotime($resume_data['upload_date'])); ?></small>
                        </div>
                    </div>

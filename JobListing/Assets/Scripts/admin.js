document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        function initDataTable(elementId, options) {
            ...
            if (element) {
                ...
                return new DataTable('#' + elementId, options);
            }
            return null;
        }
        ...
        if (saveJobBtn && !saveJobBtn.hasClickHandler) {
            saveJobBtn.hasClickHandler = true;
            saveJobBtn.addEventListener('click', async function() {
                ...
                if (form.checkValidity()) {
                    try {
                        ...
                        const requiredFields = ['title', 'company_id', 'description', 'job_type', 'slots', 'status', 'work_mode'];
                        ...
                        // Add work_mode to required fields validation
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', async function() {
                try {
                    ...
                    console.error('Error:', error);
                    let errorMessage = error.message || 'Failed to load details';
                    
                    // Check if the error is a JSON parsing error
                    if (errorMessage.includes('JSON')) {
                        errorMessage = 'Server response was not in the correct format. Please try again.';
                    }
            ...
            updateJobBtn.addEventListener('click', async function() {
                ...
                if (form.checkValidity()) {
                    try {
                        ...
                        const requiredFields = ['id', 'title', 'company_id', 'description', 'job_type', 'slots', 'status', 'work_mode'];
                        for (const field of requiredFields) {
                            const value = formData.get(field);
                            if (!value || value.trim() === '') {
                                throw new Error(`${field.replace('_', ' ')} is required`);
        ...
        await Swal.fire({
            ...
            text: 'Failed to initialize page functionality. Please refresh the page and try again.',
            icon: 'error'
        });
    }
});